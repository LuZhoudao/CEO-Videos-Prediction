        ...
        for i_batch, (batch_X, batch_Y, batch_META) in enumerate(train_loader):
            ...
            if batch_chunk > 1:
                ...
                for i in range(batch_chunk):
                    ...
                    raw_loss_i = criterion(preds_i.view(batch_size), eval_attr_i) / batch_chunk
                    raw_loss += raw_loss_i
                    raw_loss_i.backward()
                ctc_loss.backward()
                ...
                raw_loss = criterion(preds.view(batch_size), eval_attr)
                combined_loss = raw_loss + ctc_loss
    ...
    def evaluate(model, ctc_a2l_module, ctc_v2l_module, criterion, test=False):
        ...
        with torch.no_grad():
            for i_batch, (batch_X, batch_Y, batch_META) in enumerate(loader):
                ...
                #print(audio.shape, audio_mask.shape, vision)
                if hyp_params.use_cuda:
                    with torch.cuda.device(0):
                        text, audio, vision, eval_attr, text_mask, audio_mask, vision_mask = text.cuda(), audio.cuda(), vision.cuda(), eval_attr.cuda(), text_mask.cuda(), audio_mask.cuda(), vision_mask.cuda()
                        if hyp_params.dataset == 'iemocap':
                ...
                if hyp_params.dataset == 'iemocap':
                    preds = preds.view(-1, 2)
                    eval_attr = eval_attr.view(-1)
                total_loss += criterion(preds.view(batch_size), eval_attr).item() * batch_size
